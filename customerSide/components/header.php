...
$mainDishes = $pdo->query($sqlmainDishes)->fetchAll(PDO::FETCH_ASSOC);

$sqldrinks = "SELECT * FROM menu WHERE item_category = 'Drinks' ORDER BY item_type";
$drinks = $pdo->query($sqldrinks)->fetchAll(PDO::FETCH_ASSOC);

$sqlsides = "SELECT * FROM menu WHERE item_category = 'Side Snacks' ORDER BY item_type";
$sides = $pdo->query($sqlsides)->fetchAll(PDO::FETCH_ASSOC);
        ...
        <div class="nav-bar">
            <div class="brand">
                <a class="nav-link" href="../home/home.php#hero">
                    <h1 class="text-center" style="font-family:Copperplate; color:whitesmoke;"> Restaurant</h1>
                </a>
            </div>
            <div class="nav-list">
...
if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true && $account_id != null) {
    ...
    if ($row) {
        ...
        $vip_tooltip = ($vip_status === 'Regular') 
            ? ($points < 1000 ? (1000 - $points) . ' points to VIP' : 'You are eligible for VIP') 
            : '';
